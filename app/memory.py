...
class ConversationMemory:
    ...
    def get_recent(self, session_id: str, limit: int = 20) -> list[MemoryEntry]:
        history = self._history(session_id)
    ...
    def build_context(self, session_id: str, limit: int = 20) -> str:
        entries = self.get_recent(session_id, limit=limit)
        lines = [f"{entry.role.title()}: {entry.content}" for entry in entries]
        return "\n".join(lines)

    def summary(self, session_id: str, limit: int = 20) -> dict[str, int | str]:
        entries = self.get_recent(session_id, limit=limit)
